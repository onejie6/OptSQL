...
class LLM:
    ...
    def _create_client(self):
        api_key = _resolve_env_reference(self._config.api_key)
        base_url = _resolve_env_reference(self._config.base_url)
        if self._config.api_type == "openai":
            return OpenAI(api_key=api_key, base_url=base_url)
        elif self._config.api_type == "azure":
            return AzureOpenAI(api_key=api_key, base_url=base_url, api_version=self._config.api_version)
        else:
...
def _resolve_env_reference(value: str) -> str:
    """Resolve ``env:NAME`` without persisting credentials in TOML snapshots."""
    if not value.startswith("env:"):
        return value
    variable_name = value.removeprefix("env:").strip()
    resolved = os.environ.get(variable_name)
    if not resolved:
        raise RuntimeError(f"Required environment variable is not set: {variable_name}")
    return resolved
